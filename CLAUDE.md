...
- `ACOES_IA` (app.js) cobre hoje 27 ações — além de criar/editar/excluir lançamento, transferência, gasto fixo, investimento, banco, objetivo, meta, categoria, nota fiscal e cadastro de cliente/fornecedor, tem `trocar_contexto` (pessoal/empresarial pelo chat). Ferramentas novas usam o helper genérico `_acharItemIA()` pra achar o item certo por texto digitado (em vez de reescrever a lógica de busca cada vez). `api/chat-ia.js` manda no máximo 40 ferramentas pra Claude (`acoes.slice(0, 40)`) — se um dia passar disso, subir esse número.
