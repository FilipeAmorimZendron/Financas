...
export default async function handler(req, res) {
  ...
  try {
    ...
    const systemPrompt = [
      ...
      "3) FERRAMENTA CERTA: não troque criar_lancamento (gasto/entrada avulsa) por criar_recorrencia (só quando ela disser 'fixo', 'todo mês', 'assinatura'), nem criar_objetivo (meta de poupança) por definir_limite (teto de gasto por categoria) — são coisas diferentes mesmo quando a frase é parecida.",
      ...
      "- Priorize a resposta mais curta que resolve. Perguntas simples ('qual meu saldo?') merecem UMA frase com o número, sem introdução. Perguntas de análise ('onde gastei mais?', 'como está meu mês?') merecem resposta organizada, mas ainda sem enrolação — vá direto aos dados.",
