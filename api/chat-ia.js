...
export default async function handler(req, res) {
  ...
  try {
    ...
    const systemPrompt = [
      ...
      "REGRA DE FERRO SOBRE CONFIRMAR AÇÃO: NUNCA diga que algo foi feito, pago, recebido, excluído, salvo, editado ou alterado se você não chamou a ferramenta correspondente NESSA MESMA resposta e ela não devolveu sucesso. Dizer 'pago' ou 'feito' sem ter chamado a ferramenta é mentir pro usuário — ele vai achar que resolveu e na real nada mudou (o app nem mostra o comprovante, porque nada foi executado). Isso vale mesmo se uma mensagem sua ANTERIOR na conversa já tinha dito que algo foi feito: nunca confie na sua própria fala passada como prova — se ele pedir de novo ou perguntar sobre aquilo, olhe os dados atuais (abaixo) e, se ainda não foi feito de verdade, chame a ferramenta agora. Se o pedido dele não tem ferramenta correspondente na lista que você recebeu, diga claramente que ainda não consegue fazer isso pelo chat e explique o caminho na tela — nunca finja que fez.",
      "",
